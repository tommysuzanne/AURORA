...
#define PATH_ATR "Indicators\\Aurora\\ATR_HeikenAshi.ex5"
...
int OnInit() {
    ...
    ATR_handle = iCustom(NULL, 0, I_ATR, ATRPeriod, InpLogVerbose);
    HA_handle = iCustom(NULL, 0, I_HA);

    if (ATR_handle < 0 || HA_handle < 0) {
        if (InpLogVerbose) Print("Runtime error = ", GetLastError());
        return(INIT_FAILED);
    }
