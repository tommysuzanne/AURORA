   ...
   if(!allowSess)
     {
      ...
      if(session.AllowGridAddition(now))
        {
         // Allow flow for management
        }
      else
        {
   ...
   if(freeze)
     {
       ...
       // Fix: Allow continuation if Monitor Only
   ...
   if(!session.AllowTrade(now, symbol))
     {
      ...
      if(session.AllowGridAddition(now))
        {
         // RECOVERY / DELEVERAGE / SMART_HOLD
         outAllowEntry = false; // Block New Cycles
         return true;           // Allow Grid Logic (Management)
        }
      else
        {
   ...
   if(newsF.FreezeNow(now, freezeTitle, freezeCurrency))
     {
      if(newsAction == NEWS_ACTION_BLOCK_ALL_CLOSE)
       ...
       // Fix: If Monitor Only, we ignore the freeze for trading logic
