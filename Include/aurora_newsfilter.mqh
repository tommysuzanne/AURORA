...
#include <aurora_state_structs.mqh>

#define AURORA_NEWSFILTER_ADAPTER_VERSION "1.2"
#define AURORA_NEWS_DB_DIR "AURORA\\"

enum ENUM_NEWS_LEVELS
  {
   NEWS_LEVELS_NONE = 0,      // Aucune (Désactivé)
...
enum ENUM_NEWS_ACTION
  {
   NEWS_ACTION_BLOCK_ENTRIES = 0,   // Bloquer Nouvelles Entrées
   NEWS_ACTION_BLOCK_MANAGE,        // Bloquer Entrées & Gestion
   ...
   void GetUpcomingEvents(const int count, SAuroraState::SNewsItem &out_news[])
     {
      ...
      m_core.GetUpcomingEvents(TimeTradeServer(), count, evs);
